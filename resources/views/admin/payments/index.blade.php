@extends('layouts.dashboard', ['title' => 'Payment', 'userRole' => 'admin'])
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">${{ number_format($totalRevenue, 2) }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($payments as $payment)
                            <tr class="hover:bg-muted/50">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="font-medium">{{ ($payment->student_first_name ?? '') . ' ' . ($payment->student_last_name ?? '') }}</div>
                                        <div class="text-sm text-muted-foreground">{{ $payment->student_id ?? 'N/A' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded border">{{ $payment->type ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 font-semibold text-green-600">${{ number_format($payment->amount ?? 0, 2) }}</td>
                                <td class="px-6 py-4 text-sm">{{ $payment->method ?? 'N/A' }}</td>
                                <td class="px-6 py-4 text-sm">{{ $payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('M d, Y') : 'N/A' }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded-full {{ $payment->status === 'Paid' ? 'bg-green-100 text-green-800' : ($payment->status === 'Pending' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800') }}">
                                        {{ $payment->status ?? 'Pending' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <button @click="openDetailsDialog('{{ $payment->id }}')" class="text-primary hover:text-primary/80">View</button>
                                </td>
                            </tr>
                        @empty
                            <tr>

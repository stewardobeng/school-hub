@extends('layouts.dashboard', ['title' => 'Dashboard', 'userRole' => 'admin'])
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="stat-card flex items-center gap-4 animate-fade-in">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-foreground">{{ number_format($stats['totalStudents'] ?? 0) }}</p>
        </div>
    </div>
    ...
    <div class="stat-card flex items-center gap-4 animate-fade-in">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-foreground">{{ number_format($stats['totalTeachers'] ?? 0) }}</p>
        </div>
    </div>
    ...
    <div class="stat-card flex items-center gap-4 animate-fade-in">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-foreground">{{ number_format($stats['totalParents'] ?? 0) }}</p>
        </div>
    </div>
    ...
    <div class="stat-card flex items-center gap-4 animate-fade-in">
        ...
        <div>
            ...
            <p class="text-2xl font-bold text-foreground">{{ $formatEarnings($stats['totalPayments'] ?? 0) }}</p>
        </div>
    </div>
</div>
            ...
            <div class="space-y-3">
                @forelse($upcomingExams as $exam)
                    <div class="p-3 bg-muted rounded-lg">
                        <p class="font-medium text-sm">{{ $exam->title ?? $exam['title'] ?? 'N/A' }}</p>
                        <p class="text-xs text-muted-foreground mt-1">
                            {{ \Carbon\Carbon::parse($exam->exam_date ?? $exam['exam_date'] ?? now())->format('M d, Y') }}
                        </p>
                    </div>
                @empty
            ...
            <div class="space-y-3">
                @forelse($topPerformers as $performer)
                    <div class="flex items-center justify-between p-2">
                        <div>
                            <p class="font-medium text-sm">{{ ($performer->first_name ?? $performer['first_name'] ?? '') . ' ' . ($performer->last_name ?? $performer['last_name'] ?? '') }}</p>
                            <p class="text-xs text-muted-foreground">{{ $performer->grade ?? $performer['grade'] ?? 'N/A' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="font-semibold text-sm">{{ number_format($performer->avg_score ?? $performer['avg_score'] ?? 0, 1) }}%</p>
                        </div>
                    </div>
                @empty
            ...
            <div class="space-y-4">
                <div>
                    <div class="flex justify-between text-sm mb-2">
                        <span>Students</span>
                        <span>{{ $studentsPercentage }}%</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {{ $studentsPercentage }}%"></div>
                    </div>
                </div>
                <div>
                    <div class="flex justify-between text-sm mb-2">
                        <span>Teachers</span>
                        <span>{{ $teachersPercentage }}%</span>
                    </div>
                    <div class="progress-bar">
                        <div class="progress-fill" style="width: {{ $teachersPercentage }}%"></div>
                    </div>
                </div>
            </div>
        </div>

@extends('layouts.dashboard', ['title' => 'Exam', 'userRole' => 'admin'])
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $exams->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $exams->where('status', 'Scheduled')->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $exams->where('status', 'Completed')->count() }}</p>
            </div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-muted/50">
                                <td class="px-6 py-4">
                                    <div>
                                        ...
                                        <div class="text-sm text-muted-foreground mt-1">ID: {{ $exam->id }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded border font-mono">{{ $exam->course_code ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm">{{ $exam->grade }}</td>
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="font-medium">{{ $exam->exam_date ? \Carbon\Carbon::parse($exam->exam_date)->format('M d, Y') : 'N/A' }}</span>
                                        </div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded bg-muted">{{ $exam->duration ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded border">{{ $exam->type ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded-full {{ $exam->status === 'Completed' ? 'bg-green-100 text-green-800' : ($exam->status === 'In Progress' ? 'bg-blue-100 text-blue-800' : 'bg-muted') }}">
                                        {{ $exam->status ?? 'Scheduled' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <div class="flex items-center justify-end gap-2">
                                        <button @click="openEditDialog('{{ $exam->id }}')" class="text-primary hover:text-primary/80">Edit</button>
                                        <button @click="openDeleteDialog('{{ $exam->id }}')" class="text-destructive hover:text-destructive/80">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @empty

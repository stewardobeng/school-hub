@extends('layouts.dashboard', ['title' => 'Courses', 'userRole' => 'admin'])
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $courses->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $courses->where('status', 'Active')->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $courses->sum('student_count') }}</p>
            </div>
        </div>
    </div>
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($courses as $course)
                            <tr class="hover:bg-muted/50">
                                <td class="px-6 py-4">
                                    <div>
                                        <div class="font-medium">{{ $course->name ?? 'N/A' }}</div>
                                        <div class="text-sm text-muted-foreground mt-1">{{ $course->description ?? 'No description' }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded border font-mono">{{ $course->code ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm">{{ $course->grade ?? 'N/A' }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <svg class="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                        </svg>
                                        <span class="text-sm">{{ ($course->teacher_first_name ?? '') . ' ' . ($course->teacher_last_name ?? 'N/A') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded bg-muted">{{ $course->credits ?? 0 }} credits</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <svg class="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded-full {{ $course->status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-muted' }}">
                                        {{ $course->status ?? 'Active' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right text-sm">
                                    <div class="flex items-center justify-end gap-2">
                                        <button @click="openEditDialog('{{ $course->id }}')" class="text-primary hover:text-primary/80">Edit</button>
                                        <button @click="openDeleteDialog('{{ $course->id }}')" class="text-destructive hover:text-destructive/80">Delete</button>
                                    </div>
                                </td>
                            </tr>
                        @empty

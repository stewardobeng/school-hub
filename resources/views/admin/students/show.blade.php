@extends('layouts.dashboard', ['title' => 'Student Details', 'userRole' => 'admin'])

@section('content')
<div class="space-y-6" x-data="{ activeTab: 'overview' }">
    <!-- Back Button -->
    <a href="{{ route('admin.students.index') }}" class="inline-flex items-center gap-2 text-foreground hover:text-primary transition-colors">
        ...
        <div class="flex items-start gap-6">
            ...
            <div class="flex-1">
                <div class="flex items-center justify-between mb-4">
                    <div>
                        <h1 class="text-3xl font-bold mb-2">{{ $student->first_name }} {{ $student->last_name }}</h1>
                        <div class="flex items-center gap-4 text-muted-foreground">
                            <span class="font-mono text-sm">ID: {{ $student->id }}</span>
                            <span class="px-2 py-1 text-xs rounded-full {{ $student->status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $student->status }}
                            </span>
                        </div>
                    </div>
                    <a href="{{ route('admin.students.index') }}?edit={{ $student->id }}" class="bg-primary text-primary-foreground px-4 py-2 rounded-md hover:bg-primary/90 transition-colors">Edit Student</a>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm">{{ $student->grade }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm">Enrolled: {{ \Carbon\Carbon::parse($student->enrollment_date)->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-card rounded-xl p-4 shadow-sm border border-border/50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ number_format($overallGPA, 2) }}</p>
                </div>
        ...
        <div class="bg-card rounded-xl p-4 shadow-sm border border-border/50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold">{{ number_format($attendanceRate, 1) }}%</p>
                </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-medium">{{ \Carbon\Carbon::parse($student->enrollment_date)->format('M d, Y') }}</p>
                        </div>
                        <div>
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($courses as $course)
                            <tr>
                                ...
                                <td class="px-6 py-4">{{ $course->teacher_name ?? 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $course->credits ?? 0 }}</td>
                            </tr>
                        @empty
                            <tr>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($examResults as $result)
                            <tr>
                                <td class="px-6 py-4 font-medium">{{ $result->title ?? 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $result->exam_date ? \Carbon\Carbon::parse($result->exam_date)->format('M d, Y') : 'N/A' }}</td>
                                <td class="px-6 py-4">
                                    <span class="font-semibold">{{ $result->score ?? 0 }}/{{ $result->max_score ?? 100 }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $grade = 'N/A';
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 text-xs rounded {{ $grade === 'A' ? 'bg-green-100 text-green-800' : 'bg-muted' }}">{{ $grade }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($attendance as $record)
                            <tr>
                                <td class="px-6 py-4 font-medium">{{ $record->attendance_date ? \Carbon\Carbon::parse($record->attendance_date)->format('M d, Y') : 'N/A' }}</td>
                                <td class="px-6 py-4">{{ $record->class_name ?? 'N/A' }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded {{ $record->status === 'Present' ? 'bg-green-100 text-green-800' : ($record->status === 'Late' ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800') }}">
                                        {{ $record->status ?? 'N/A' }}
                                    </span>
                                </td>
                            </tr>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-border">
                        @forelse($payments as $payment)
                            <tr>
                                <td class="px-6 py-4 font-mono text-sm">{{ $payment->id }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded border">{{ $payment->payment_type ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 font-semibold text-green-600">${{ number_format($payment->amount ?? 0, 2) }}</td>
                                <td class="px-6 py-4">{{ $payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('M d, Y') : 'N/A' }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 text-xs rounded bg-green-100 text-green-800">{{ $payment->status ?? 'Paid' }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>

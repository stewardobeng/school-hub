@extends('layouts.dashboard', ['title' => 'Students', 'userRole' => 'admin'])

@section('content')
<div class="space-y-6" x-data="studentManagement()" x-init="@if(request()->has('edit')) $nextTick(() => { openEditDialog('{{ request()->get('edit') }}'); }); @endif">
    <!-- Stats Row -->
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $students->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $students->where('status', 'Active')->count() }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $students->filter(function($s) { return \Carbon\Carbon::parse($s->enrollment_date)->isCurrentMonth(); })->count() }}</p>
            </div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-muted/50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="font-medium text-foreground">{{ $student->first_name }} {{ $student->last_name }}</div>
                                            <div class="text-sm text-muted-foreground">{{ $student->email }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap font-mono text-sm">{{ $student->id }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="space-y-1">
                                        <div class="flex items-center gap-2 text-sm">
                                            ...
                                            <span class="text-muted-foreground">{{ $student->email ?? 'N/A' }}</span>
                                        </div>
                                        <div class="flex items-center gap-2 text-sm">
                                            <svg class="w-3 h-3 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                                            </svg>
                                            <span class="text-muted-foreground">{{ $student->phone ?? 'N/A' }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">{{ \Carbon\Carbon::parse($student->enrollment_date)->format('M d, Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.students.show', $student->id) }}" class="text-primary hover:text-primary/80">View</a>
                    ...
                    <tbody class="divide-y divide-border" x-show="searchQuery !== ''" x-cloak>
                        <template x-for="student in filteredStudents" :key="student.id">
                            <tr class="hover:bg-muted/50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    <div class="flex items-center justify-end gap-2">
                                        <a :href="`{{ route('admin.students.index') }}/${student.id}`" class="text-primary hover:text-primary/80">View</a>
                                        <button @click="openEditDialog(student.id)" class="text-primary hover:text-primary/80">Edit</button>
                                        <button @click="openDeleteDialog(student.id)" class="text-destructive hover:text-destructive/80">Delete</button>
                                    </div>
                                </td>
...
function studentManagement() {
    return {
        ...
        filteredStudents: @json($students),
        isLoading: false,
        errorMessage: '',
        successMessage: '',
        openAddDialog() {

@extends('layouts.dashboard', ['title' => 'Attendance', 'userRole' => 'admin'])
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ number_format($overallAttendance, 1) }}%</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $todayPresent }}</p>
            </div>
        </div>
        <div class="stat-card flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-foreground">{{ $todayAbsent }}</p>
            </div>
        </div>
    </div>
                            ...
                            <tbody class="divide-y divide-border">
                                @forelse($attendance as $record)
                                    ...
                                    <tr class="hover:bg-muted/50">
                                        ...
                                        <td class="px-6 py-4 text-muted-foreground">{{ ($record->teacher_first_name ?? '') . ' ' . ($record->teacher_last_name ?? '') }}</td>
                                        <td class="px-6 py-4">{{ $record->total_students ?? 0 }}</td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <svg class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                </svg>
                                                <span class="font-medium text-green-600">{{ $record->present ?? 0 }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="font-medium text-red-600">{{ $record->absent ?? 0 }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="font-medium text-amber-600">{{ $record->late ?? 0 }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <span class="px-2 py-1 text-xs rounded-full {{ $record->status === 'Completed' ? 'bg-green-100 text-green-800' : 'bg-muted' }}">
                                                    {{ $record->status ?? 'Pending' }}
                                                </span>
                                                <span class="text-xs text-muted-foreground">({{ number_format($attendanceRate, 1) }}%)</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-right text-sm">
                                            <div class="flex items-center justify-end gap-2">
                                                <button @click="openEditDialog('{{ $record->id }}')" class="text-primary hover:text-primary/80">Edit</button>
                                                <button @click="openDeleteDialog('{{ $record->id }}')" class="text-destructive hover:text-destructive/80">Delete</button>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
